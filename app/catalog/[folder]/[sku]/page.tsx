    ...
    <div className="pb-16">
      ...
      <section className="page-hero">
        <div className="page-hero-inner">
          ...
          {customContent && (
            <div className="mt-5 max-w-3xl rounded-2xl border border-kraft-300/60 bg-white/70 p-4">
              <p className="text-base font-black text-brand-600">{customContent.headline}</p>
          ...
          <div className="mt-8 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl bg-[rgba(255,255,255,0.62)] p-4">
              ...
              <p className="mt-1 text-2xl font-black tracking-[-0.03em] text-brand-600">{money(product.startingPrice)}</p>
              <p className="text-sm text-muted">per case</p>
            </div>
            <div className="rounded-2xl bg-[rgba(255,255,255,0.62)] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.08em] text-accent-600">Availability</p>
              <p className="mt-1 text-lg font-black text-brand-600">
                {product.availability === 'stock' ? 'In Stock' : 'Custom Print'}
              </p>
              <p className="text-sm text-muted">{getLeadTimeShort(product.availability)}</p>
            </div>
            <div className="rounded-2xl bg-[rgba(255,255,255,0.62)] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.08em] text-accent-600">Available Sizes</p>
              <p className="mt-1 text-lg font-black text-brand-600">{sizes.length}</p>
              <p className="text-sm text-muted">size option{sizes.length === 1 ? '' : 's'}</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="tonal-panel">
            ...
            <div className="mt-5 grid gap-3 text-sm text-muted">
              <div className="rounded-2xl border border-kraft-300/40 bg-white p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.08em] text-accent-600">SKU</p>
                <p className="mt-1 font-semibold text-brand-600">{product.sku}</p>
              </div>
              <div className="rounded-2xl border border-kraft-300/40 bg-white p-4">
              ...
              <div className="rounded-2xl border border-kraft-300/40 bg-white p-4">
                ...
                <p className="mt-1 font-semibold text-brand-600">{getLeadTimeShort(product.availability)}</p>
              </div>
            </div>

            {customContent && (
              <div className="mt-5 grid gap-3">
                <div className="rounded-2xl border border-kraft-300/40 bg-[#FCF8F2] p-4">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {customContent.features.map((feature) => (
                    <div key={feature} className="rounded-2xl border border-kraft-300/40 bg-white p-4 text-sm leading-6 text-muted">
                      {feature}
                    </div>
                  ))}
                </div>
            ...
            {sizes.map((size) => {
              ...
              return (
                <article key={size.slug} className="surface-card product-card">
                  ...
                  <div className="p-4">
                    <p className="text-xs font-black uppercase tracking-[0.08em] text-accent-600">Size Option</p>
                    <h3 className="mt-2 font-serif text-base text-brand-600">{size.label}</h3>
                    <div className="mt-3 flex items-end justify-between gap-3 rounded-2xl border border-kraft-300/40 bg-white p-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.08em] text-accent-600">Pricing</p>
