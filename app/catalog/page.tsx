...
const collections = [
  { name: 'Custom Printed Bags', slug: 'custom' },
  { name: 'Dispensary Bags', slug: 'dispensary' },
  { name: 'Faith & Religion Bags', slug: 'faith' },
  { name: 'Holiday Bags', slug: 'holiday' },
  { name: 'Pharmacy Bags', slug: 'pharmacy' },
  { name: 'Pride Bags', slug: 'pride' },
  { name: 'USA Bags', slug: 'usa' },
  { name: 'Veterinary Bags', slug: 'veterinary' },
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {collections.map((c) => (
              <Link
                key={c.slug}
                href={c.slug === 'pharmacy' ? '/catalog/pharmacy' : c.slug === 'veterinary' ? '/catalog/veterinary' : c.slug === 'custom' ? '/catalog/custom' : `/contact?collection=${c.slug}`}
                className="px-6 py-3 bg-white border-2 border-gray-200 rounded-xl font-semibold text-gray-700 hover:bg-primary-50 hover:border-primary-300 hover:text-primary-700 transition-all shadow-md hover:shadow-lg"
              >
                {c.name}
              </Link>
            ))}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {customProducts.map((product, index) => {
              ...
              return (
                <div
                  key={product.id}
                  className="bg-white border-2 border-gray-100 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2"
                >
