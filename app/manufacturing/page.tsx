  ...
  return (
    <div className="section-container py-14 md:py-20">
      <div className="max-w-4xl">
        ...
        <h1 className="heading-serif mt-5 text-4xl font-black text-slate-900 md:text-5xl">Factory Direct Manufacturing Authority</h1>
      ...
      <div className="mt-8 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {advantages.map((advantage) => (
          <div key={advantage} className="surface-card rounded-lg p-4 font-semibold text-slate-800">
            {advantage}
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        {photos.map((src) => (
          <div key={src} className="relative h-52 overflow-hidden rounded-lg border border-slate-200 shadow-sm">
      ...
      <div className="mt-8 flex flex-wrap gap-4 text-sm font-semibold text-slate-700">
        <Link href="/made-in-usa-paper-bags" className="rounded-md bg-white px-3 py-2 underline">
          Made in USA paper bags
        </Link>
        <Link href="/pharmacy-paper-bags-wholesale" className="rounded-md bg-white px-3 py-2 underline">
